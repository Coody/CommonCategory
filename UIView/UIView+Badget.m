...
-(instancetype)initWithAppendedView:(UIView *)tempView withBadgetImage:(UIImage *)badgetImage{
    self = [super init];
    if ( self ) {
        _showBadget = YES;
...
- (UIImage *)badgetImage 
{
    return objc_getAssociatedObject(self, @selector(badgetImage));
}

- (void) setBadgetImage:(UIImage *)badgetImage
{
    objc_setAssociatedObject(self, @selector(badgetImage), badgetImage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(void)createBadget{
    if ( self.badgetView == nil ) {
        self.badgetView = [[BadgetView alloc] initWithAppendedView:self withBadgetImage:self.badgetImage];
